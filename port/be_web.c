...
void be_run_end(const char *ev, const char *killer, long score)
{
    ...
    js_beacon("arogue77", ev, whoami, killer, level, (int) score, (int) turns, pstats.s_lvl);
}
...
{
    int fd, y, x;
    if (!file_name[0] || pstats.s_hpt <= 0 || (fd = open(file_name, O_WRONLY|O_CREAT|O_TRUNC, 0666)) < 0) return;
    getyx(cw, y, x);
    save_file(fd);              /* closes fd */
    wmove(cw, y, x);
    draw(cw);
}
