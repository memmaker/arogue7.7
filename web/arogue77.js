/*
 * Advanced Rogue 7.7 in the browser: draws the panes of the curses shim (port/be_web.c
 * calls Module.xr), keyboard input, tiling windows, sound, saves in
 * IndexedDB. Loaded before arogue77-core.js.
 */
(function () {
	'use strict';
	...
	var DIR = '/arogue77/save';                         /* IDBFS mount: saves, scores, layout */
	var SAVE = DIR + '/arogue77.sav', LAYOUT_FILE = DIR + '/web-layout.json';
	...
	window.Module = {
		...
		preRun: [function () {
			...
			FS.chdir('/arogue77');
			Module.ENV.HOME = DIR;               /* arogue77.sav (md_gethomedir) */
			Module.ENV.ROGUEHOME = DIR;          /* score file */
			Module.ENV.USER = 'rogue';
			var who = '';                        /* whoami: getpwuid() is web_user, so ask once */
			try { who = localStorage.getItem('arogue77-name') || ''; } catch (err) { /* no storage */ }
			if (!who) { who = (prompt('What is your name, adventurer?', '') || '').replace(/[,\n]/g, '').trim().slice(0, 30); try { if (who) localStorage.setItem('arogue77-name', who); } catch (err) { /* no storage */ } }
	...
	function crashed(err) {
		...
		console.error('[arogue77] crash:', err);
	...
	window.addEventListener('error', function (e) {
		...
		if (e.error instanceof WebAssembly.RuntimeError || /arogue77-core/.test(e.filename || '')) crashed(e.error || e.message);
	});
